...
async function fetchProfile(steamid) {
  ...
  // 5 Looking to trade, 6 Looking to play. gameextrainfo/gameid are only
  // present when actually in a game — this was missing for the user's
  // OWN profile (the friends-list fetch already had it), which is why
  // online/in-game status only ever showed up for friends.
  const personaStates = {
    0: "offline",
    1: "online",
    2: "busy",
...
async function fetchAppNames(appids) {
  const fresh =
    appListCache && Date.now() - appListCache.fetchedAt < APP_LIST_TTL_MS;
  if (!fresh) {
    try {
...
async function fetchWishlist(steamid) {
  ...
  return {
    source: games.length > 0 ? "legacy" : "both-empty",
    games: games.sort((a, b) => a.priority - b.priority),
  };
}
