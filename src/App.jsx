...
export default function App() {
  ...
  const stats = useMemo(
    () => ({
      ...
      platforms: new Set(giveaways.flatMap((g) => (g.platforms || "").split(",").map((s) => s.trim().toLowerCase()))).size,
  ...
  return (
    <>
      ...
      <div className="[@media(max-width:450px)_and_(max-height:450px)]:hidden">
        <div className="min-h-screen font-sans theme-transition" style={{ background: theme.pageBg, color: theme.text }}>
          <AmbientBackground />
          <Nav />
          <Hero stats={stats} />
      ...
      <div className="hidden [@media(max-width:450px)_and_(max-height:450px)]:block">
        <WatchView giveaways={filtered} platformFilter={platformFilter} setPlatformFilter={setPlatformFilter} status={status} />
      </div>
    </>
  );
}
