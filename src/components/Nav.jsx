...
import SteamMenu from "./SteamMenu.jsx";
import { useSteam } from "../context/SteamContext.jsx";

export default function Nav() {
  const { theme, dark, setDark } = useTheme();
  const { connected } = useSteam();

  return (
    <nav
      className="relative z-20 backdrop-blur-xl border-b-2 theme-transition"
      style={{ background: theme.navBg, borderColor: theme.navBorder }}
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2 min-w-0 cursor-pointer">
          <div
          ...
          <span
            className="font-black tracking-tight text-base sm:text-lg truncate"
        ...
        <div className="flex items-center gap-2 shrink-0">
          {connected ? <SteamMenu /> : <SteamConnect />}
          <div
            className="flex items-center gap-1 p-1 rounded-full border-2 shrink-0"
            style={{
